          ...
          <table class="table table-striped-columns">
            ...
            <tbody>
                <?php 
                    ...
                    $search = mysqli_query($con, "SELECT COUNT(*) as total_records FROM `orderconfirmation` WHERE `status` = 0 ORDER BY `oc_id` DESC")
                    or die(mysqli_errno($con));
                    $records = mysqli_fetch_array($search);
                    $total_records = $records['total_records'];
                    $total_no_of_pages = ceil($total_records / $total_records_per_page);

                    $getUsersInfo = mysqli_query($con,"SELECT * FROM `orderconfirmation`  WHERE `status` = 0 ORDER BY `oc_id` DESC LIMIT $offset , $total_records_per_page");
                    ...
                    <tr>
                        ...
                        <?php
                            if ($row['status'] == 0 ) {
                            echo '<span class="badge bg-secondary text-white  mt-1 p-2"><i class="fa fa-exclamation-triangle"></i> Ready to Pick-Up </span>';
                            } else {
                            echo '<span class="badge bg-success mt-1 p-2"><i class="fa fa-check-circle"></i> Picked-up </span>';
                            }
                        ?>
                        </td>
                        <td>
